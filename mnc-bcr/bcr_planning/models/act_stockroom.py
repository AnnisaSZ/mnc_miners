...
class ActStockroom(models.Model):
    _name = 'act.stockroom'
    _description = 'Stock Room'
    _rec_name = "kode"

    # Master
    kode = fields.Char(string='Kode StockRoom', readonly=True, default="#")
    activity_id = fields.Many2one('master.activity', string='Activity', required=True,
                                  default=lambda self: (self.env["master.activity"].get_activity_by_code('01-SR')))
    ...
    validation_plan = fields.One2many('validation.plan', 'validation_act_stockroom_id', string='Validation',
    ...
    def create(self, vals):
        res = super(ActStockroom, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('act.stockroom')
    ...
    def set_validation(self):
        ...
        if bu_id:
            ...
            if validation:
                ...
                raise UserError('Validation untuk Form StockRoom belum disetting')
