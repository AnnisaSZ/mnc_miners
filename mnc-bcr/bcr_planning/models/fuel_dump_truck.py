...
class FuelDumpTruck(models.Model):
    _name = 'fuel.dump.truck'
    _description = 'Fuel Dump Truck'
    _rec_name = "kode"
    ...
    ritase = fields.Float(string="Ritase", required=True, help="Total Ritase Kendaraan pengangkutan")
    ...
    validation_plan = fields.One2many('validation.plan', 'validation_fuel_dump_truck_id', string='Validation',
    ...
    def create(self, vals):
        res = super(FuelDumpTruck, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('fuel.dump.truck')
