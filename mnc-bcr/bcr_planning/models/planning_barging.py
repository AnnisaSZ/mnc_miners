...
class PlanningBarging(models.Model):
    _name = 'planning.barging'
    _description = 'Planning Barging'
    ...
    activity_id = fields.Many2one('master.activity', string='Activity', required=True,
                                  default=lambda self: (self.env["master.activity"].get_activity_by_code('01-BG')))
    ...
    gar = fields.Float('GAR', required=True, default=0)
    volume_plan = fields.Float('Volume Plan', required=True, default=0)
    ts = fields.Float('TS', default=0)
    ash = fields.Float('Ash', default=0)
    tm = fields.Float('TM', default=0)
    im = fields.Float('IM', default=0)
    ...
    validation_plan = fields.One2many('validation.plan', 'validation_planning_barging_id', string='Validation',
    ...
    def create(self, vals):
        res = super(PlanningBarging, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('planning.barging')
    ...
    def _compute_workdays(self):
        if self.date_start and self.date_end:
            ...
            # d5 = str(d5.days + 1)
            # if int(d5) <= -2:
            #     raise UserError('Input data Backdate hanya boleh maksimal 1 hari')
    ...
    #tambahkan backend validation juga disini
    #tambahkan frontend validation di xml