...
class ActProduction(models.Model):
    _name = 'act.production'
    ...
    activity_id = fields.Many2one('master.activity', string='Activity', required=True,
                                  default=lambda self: (self.env["master.activity"].get_activity_by_code('01-PR')))
    ...
    source_id = fields.Many2one('master.source', string='Source', required=True)
    ...
    validation_plan = fields.One2many('validation.plan', 'validation_act_production_id', string='Validation',
    ...
    # New CR 0004/Production/XXVII/IX/2023
    distance = fields.Float('Distance', default=0.0, store=True, required=True)
    # =================================
    ...
    def create(self, vals):
        res = super(ActProduction, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('act.production')
    ...
    def set_validation(self):
        ...
        if bu_id:
            ...
            if validation:
                ...
                raise UserError('Validation untuk Form Production belum disetting')
