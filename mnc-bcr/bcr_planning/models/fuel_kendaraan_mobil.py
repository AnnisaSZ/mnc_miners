...
class FuelKendaraanMobil(models.Model):
    _name = 'fuel.kendaraan.mobil'
    _description = 'Fuel Kendaraan Mobil'
    _rec_name = "kode"
    ...
    unit_id = fields.Many2one('master.unit.kendaraan', string="Unit Kendaraan", required=True,
                              help="Nama Kendaraan/Alat yang melakukan pengisian bahan bakar. Kode Unit Kendaraan, (EXC = EXCAVATOR, DT = Dump Truck)")
    tipe_unit_id = fields.Many2one('tipe.unit.kendaraan', string="Tipe Unit Kendaraan", required=True, help="Jenis Kendaraan/Alat yang melakukan pengisian bahan bakar")
    driver_id = fields.Many2one('master.driver', string="Driver",
                                help="Nama Driver yang melakukan pengisian Bahan Bakar")
    no_pengambilan = fields.Integer(string="No Pengambilan")
    no_polisi = fields.Char(string="No Polisi", help="Nomor Polisi Kendaraan")
    km_awal = fields.Float(string="KM Awal", help="Kilometer Awal")
    km_akhir = fields.Float(string="KM Akhir", help="Kilometer Akhir")
    ...
    jarak_tempuh = fields.Float(string="Jarak Tempuh (Km)", compute='_compute_jarak_tempuh', store=True, help="Km Awal - Km Akhir")
    konsumsi_bbm = fields.Float(string="KONSUMSI BBM (Km/L)", compute='_compute_konsumsi_bbm', store=True, help="Jarak Tempuh / BBM (L)")
    ...
    def _compute_jarak_tempuh(self):
        if self.km_awal and self.km_akhir:
            self.jarak_tempuh = self.km_akhir - self.km_awal

    @api.depends('km_awal', 'km_akhir', 'bbm')
    def _compute_konsumsi_bbm(self):
        if self.km_awal and self.km_akhir and self.bbm:
            self.konsumsi_bbm = (self.km_akhir - self.km_awal) / self.bbm

    # create, sequence
    @api.model
    def create(self, vals):
        res = super(FuelKendaraanMobil, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('fuel.kendaraan.mobil')
    ...
    def set_validation(self):
        ...
        if bu_id:
            ...
            if validation:
                ...
                raise UserError('Validation untuk Form Fuel Kendaraan Mobil belum disetting')
