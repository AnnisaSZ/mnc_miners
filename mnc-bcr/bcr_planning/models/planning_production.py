...
class PlanningProduction(models.Model):
    _name = 'planning.production'
    ...
    plan_mine_exposed = fields.Float('Planning Mine Exposed', required=True, default=0)
    ...
    validation_plan = fields.One2many('validation.plan', 'validation_planning_production_id', string='Validation',
    ...
    def create(self, vals):
        res = super(PlanningProduction, self).create(vals)
        seq = self.env['ir.sequence'].next_by_code('planning.production')
        _logger.info(seq)
        seq_code = self.env["res.users"].get_default_bisnis_unit_seq_code()
        _logger.info(seq_code)
