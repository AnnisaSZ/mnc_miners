...
class SLPApproval(models.Model):
    _name = 'slp.approval'
    _description = 'Waiting Approval'
    _order = 'id asc'

    name = fields.Char('Name', compute='get_name_joined', store=True)
    slp_id = fields.Many2one('submission.letter.payment', string='SLP', ondelete='cascade')
    ...
    def get_name_joined(self):
        for approval in self:
            name = '/'
            if approval.slp_id:
                name = approval.slp_id.name
            elif approval.slp_id:
                name = approval.slp_id.name
            approval.name = name
