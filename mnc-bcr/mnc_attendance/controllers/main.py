...
def _set_check_in_out(self, employee_id, working_time, dayofweek):
    datas = {}
    calendar_att_obj = request.env['resource.calendar.attendance']
    resouce_line_id = working_time.attendance_ids.filtered(lambda x: dict(calendar_att_obj._fields['dayofweek'].selection).get(x.dayofweek) == dayofweek)
    if resouce_line_id:
        start_ci = resouce_line_id.start_ci
        start_co = resouce_line_id.start_co
        datas['id'] = employee_id.working_time_id.id
        datas['start_ci'] = float_to_time_str(start_ci)
        datas['start_co'] = float_to_time_str(start_co) if start_co != 0.0 else None
        # Working Time
        time_start = get_time(self, resouce_line_id.hour_from)
        time_end = get_time(self, resouce_line_id.hour_to)
        datas['working_time'] = _("%s - %s") % (time_start, time_end)
        datas['is_working'] = True
    else:
        start_ci = 0.0
        start_co = 0.0
        datas['id'] = employee_id.working_time_id.id
        datas['start_ci'] = float_to_time_str(start_ci)
        datas['start_co'] = float_to_time_str(start_co)
        # Working Time
        time_start = get_time(self, start_ci)
        time_end = get_time(self, start_co)
        datas['working_time'] = _("%s - %s") % (time_start, time_end)
        datas['is_working'] = False
    return datas


# Penyederhanaan fungsi
def _get_working_time(self, employee_id, dates):
    working_time = {}
    dayofweek = dates.strftime("%A")
    if not employee_id.shift_temp_ids:
        if employee_id.working_time_id:
            working_time = _set_check_in_out(self, employee_id, employee_id.working_time_id, dayofweek)
        else:
            result = {
                "code": 3,
                "data": [],
        ...
        attendance_id = employee_id.shift_temp_ids.filtered(lambda x: x.start_date <= dates and x.end_date >= dates)
        if attendance_id:
            working_time = _set_check_in_out(self, employee_id, attendance_id.working_time_id, dayofweek)
        else:
            working_time = _set_check_in_out(self, employee_id, employee_id.working_time_id, dayofweek)
    return working_time
    ...
    def getDataAccount(self, values=None):
        ...
        if res['code'] == 2:
            ...
            if employee_id:
                ...
                working_time = _get_working_time(self, employee_id, today)
                data_shift['start_ci'] = working_time['start_ci']
                data_shift['start_co'] = working_time['start_co']
            # Check HR
    ...
    def getDates(self, values=None):
        if request.httprequest.headers.get('Api-key'):
            ...
            if key == api_key:
                ...
                time_today = (fields.Datetime.now() + timedelta(hours=7))

                if time_today.hour >= start_time.hour:
                    # Ambil Hari ini
                    date_check_in = (datetime.strptime(f"{time_today.date()} {waktu}", "%Y-%m-%d %H:%M:%S")) - timedelta(hours=7)
                    date_check_out = (date_check_in + timedelta(days=1))
                else:
                    # Ambil Kemarin
                    date_check_in = (start_time - timedelta(days=1)) - timedelta(hours=7) # tgl 6 jam 22
                    date_check_out = start_time - timedelta(hours=7) # tgl 7 jam 22
                # Get Chech In
                domain_ci = domain + [('check_in', '>=', date_check_in), ('check_in', '<=', date_check_out)]
                attendance_ci = request.env['hr.attendance'].sudo().search(domain_ci)
                # Get Chech Out
            ...
            if key == api_key:
                ...
                if not att_id:
                    ...
                    if att_id.resouce_line_id:
                        ...
                        time_start = get_time(self, 0.0)
                        time_end = get_time(self, 0.0)
                        working_time['working_time'] = _("%s - %s") % (time_start, time_end)
                    ...
                    if att_id.check_in:
                        ...
                        # 'time_check_out': remove_seconds(attendance_id.check_out, attendance_id.type_co) or "",
                        detail['check_in'] = {
                            'date_ci': (att_id.check_in + timedelta(hours=7)).date() or "",
                    ...
                    if att_id.check_out:
                        detail['check_out'] = {
                            'date_co': (att_id.check_out + timedelta(hours=7)).date() or "",
