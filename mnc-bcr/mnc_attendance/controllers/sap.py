...
def parameter_sap_list(sap_id):
    code_comp = request.env['master.bisnis.unit'].sudo().search(
        [('bu_company_id', '=', sap_id.company_id.id)], limit=1).code or False
    # eviden_url = _("%web/image?model=sapform&id=%s&field=img_eviden") % (url, str(sap_id.id))
    # eviden_result_url = _("%sweb/image?model=sapform&id=%s&field=img_eviden_result") % (url, str(sap_id.id))
        ...
        if request.httprequest.headers.get('Api-key'):
            ...
            if key == api_key:
                ...
                if parameter['month'] > 0 and parameter['month'] < 13 and "draft" not in parameter['state']:
                    ...
                    end_date = datetime(parameter['year'], parameter['month'], calendar.mdays[parameter['month']]) + timedelta(days=1)
        ...
        if request.httprequest.headers.get('Api-key'):
            ...
            if sap:
                ...
                if parameter['code'] == 1:
                    ...
                    if sap.child_pic:
                        for user in sap.pic_id.pic_ids:
                            user = user
                            sap.send_notification(user, "HSE-06", sap)
                    return {
                        "code": 2,
                        "desc": "Success",
                        "data": "SAP is closed"
    ...
    def ReviseSAP(self, values=None):
        if request.httprequest.headers.get('Api-key'):
            ...
            if sap_id:
                ...
                if parameter['code'] == 1:
                    if request.env.user.id not in sap_id.pic_id.pic_ids.ids:
                        return {
                            "code": 3,
                            "desc": "Failed",
                            "data": "Your not PIC or HSE"
                        }
                    elif request.env.user.has_group('mnc_sap.group_hse') or request.env.user.id in sap_id.pic_id.pic_ids.ids:
